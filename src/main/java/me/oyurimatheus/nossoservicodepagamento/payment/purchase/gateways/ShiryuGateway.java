...
import static me.oyurimatheus.nossoservicodepagamento.payment.purchase.gateways.CreditCardFlag.MASTERCARD;
import static me.oyurimatheus.nossoservicodepagamento.payment.purchase.gateways.CreditCardFlag.VISA;

@Service
class ShiryuGateway implements PaymentGateway {

    @Value("${nosso-servico-de-pagamentos.gateways.shiryu}")
    private BigDecimal fare;
    private final Country country = BR;
    private final PaymentGatewayClient client;
    private EnumSet<CreditCardFlag> cardFlags = EnumSet.of(VISA, MASTERCARD);

    ShiryuGateway(PaymentGatewayClient client) {
        this.client = client;
    }
