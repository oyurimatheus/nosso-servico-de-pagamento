...
@Service
class SaoriGateway implements PaymentGateway {

    @Value("${nosso-servico-de-pagamentos.gateways.saori}")
    private BigDecimal fare;
    private final Country country = BR;
    private final PaymentGatewayClient client;
    private EnumSet<CreditCardFlag> cardFlags = EnumSet.allOf(CreditCardFlag.class);

    SaoriGateway(PaymentGatewayClient client) {
        this.client = client;
    }
    ...
    public BigDecimal cost(Payment payment) {
        return fare.multiply(payment.getTotal());
    }
